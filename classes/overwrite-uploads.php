...
class OverwriteUploads {
	...
	public function remove_existing_attachment( $file ) {
		...
		if(function_exists('cud_wp_upload_dir') && function_exists('cud_generate_dir')){
			$uploads_dir = array( 
				'path' => cud_wp_upload_dir() . cud_generate_dir(),
				'subdir' => str_ireplace('\\', '/', cud_generate_dir())
			);
		}
		
		else $uploads_dir = wp_upload_dir();
		
		if ( file_exists( $uploads_dir['path'] . '/' . $file['name'] ) ) {
			$params = array(
				'numberposts'   => 1,
				...
				'meta_query'    => array(
					array(
						...
						'value' => trim( $uploads_dir['subdir'] . '/' . $file['name'], '/' )
					)
				)
			);
